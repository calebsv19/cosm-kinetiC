...
static bool test_boundary_enforcement_zeros_only_boundary_cells(void) {
    ...
    if (!sim_runtime_backend_3d_test_write_cell(backend, center_x, center_y, 0, 5.0f, 2.0f, 3.0f, 4.0f, 1.0f, 1u)) {
        return false;
    }

    if (!sim_runtime_backend_3d_test_write_cell(
            backend, center_x, center_y, center_z, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 0u)) {
        return false;
    }
...
static bool test_brick_local_obstacles_survive_zeroed_dense_cache(void) {
    AppConfig cfg = {0};
    SimModeRoute route = {
        .backend_lane = SIM_BACKEND_CONTROLLED_3D,
        .requested_space_mode = SPACE_MODE_3D,
    ...
    if (!sim_runtime_backend_3d_test_write_cell(backend, center_x, center_y, 0, 5.0f, 1.0f, 2.0f, 3.0f, 4.0f, 1u)) {
        return false;
    }
    if (!sim_runtime_backend_3d_test_write_cell(
            backend, center_x, center_y, center_z, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 0u)) {
    ...
    if (!sim_runtime_backend_3d_test_zero_obstacle_dense_cache(backend)) return false;

    sim_runtime_backend_enforce_obstacles(backend, NULL);
    if (!sim_runtime_backend_3d_test_view_refresh(backend, &impl)) return false;

    if (impl.obstacle_occupancy[boundary_idx] == 0u) return false;
    if (impl.volume.density[boundary_idx] != 0.0f) return false;
    if (impl.volume.velocity_x[boundary_idx] != 0.0f) return false;
    if (impl.volume.velocity_y[boundary_idx] != 0.0f) return false;
    if (impl.volume.velocity_z[boundary_idx] != 0.0f) return false;
    ...
    if (!sim_runtime_backend_get_obstacle_view_2d(backend, &obstacle)) return false;
    obstacle_slice_idx = (size_t)center_y * (size_t)obstacle.width + (size_t)center_x;
    if (obstacle.solid_mask[obstacle_slice_idx] != 0u) return false;
    if (obstacle.solid_mask[0] == 0u) return false;
...
int main(void) {
    ...
    if (!test_brick_local_obstacles_survive_zeroed_dense_cache()) {
        fprintf(stderr,
                "sim_runtime_backend_3d_obstacle_contract_test: brick-local obstacle authority failed\n");
        return 1;
    }
