...
#define TRIO_CONTRACT_DIR PHYSICS_SIM_REPO_ROOT "/third_party/codework_shared/assets/scenes/trio_contract"
...
static bool test_runtime_scene_bridge_preflight_accepts_runtime_fixture(void) {
    ...
    bool ok = runtime_scene_bridge_preflight_file(TRIO_CONTRACT_DIR "/scene_runtime_min.json",
                                                  &preflight);
    if (!ok) return false;
...
static bool test_runtime_scene_bridge_rejects_authoring_variant(void) {
    RuntimeSceneBridgePreflight preflight;
    bool ok = runtime_scene_bridge_preflight_file(TRIO_CONTRACT_DIR "/scene_authoring_min.json",
                                                  &preflight);
    if (ok) return false;
...
static bool test_runtime_scene_bridge_apply_fixture(void) {
    ...
    bool ok = runtime_scene_bridge_apply_file(TRIO_CONTRACT_DIR "/scene_runtime_min.json",
                                              &cfg,
                                              &preset,
                                              &summary);
    if (!ok) return false;
...
static bool test_runtime_scene_bridge_trio_fixture_compile_writeback_apply(void) {
    ...
    char *authoring_json = read_text_file_alloc(TRIO_CONTRACT_DIR "/scene_authoring_interop_min.json",
                                                &authoring_size);
    char *overlay_json = read_text_file_alloc(TRIO_CONTRACT_DIR "/physics_overlay_min.json",
                                              &overlay_size);
    char diagnostics[256];
    char *runtime_json = NULL;
