...
static void expect_true(const char *name, bool condition) {
    if (!condition) {
        fprintf(stderr, "physics_sim_path_opener_test: failed: %s\n", name);
        failures += 1;
    }
}
...
static bool setup_fixture(char *root, char *directory, char *xdg_open, char *gio, char *log_path) {
    char root_template[] = "/tmp/physics_sim_path_opener_XXXXXX";
    const char *xdg_script =
        "#!/bin/sh\n"
        "printf '%s\\n' \"$@\" > \"$PHYSICS_SIM_PATH_OPENER_LOG\"\n"
        "case \"$PHYSICS_SIM_PATH_OPENER_XDG\" in\n"
...
static void test_xdg_open_receives_directory(void) {
    ...
    if (ready) {
        ...
        expect_true("opener_xdg_log", setenv("PHYSICS_SIM_PATH_OPENER_LOG", log_path, 1) == 0);
        expect_true("opener_xdg_mode", setenv("PHYSICS_SIM_PATH_OPENER_XDG", "opened", 1) == 0);
        expect_true("opener_xdg_result", PhysicsSim_PathOpener_OpenDirectory(directory) == PHYSICS_SIM_PATH_OPENER_OPENED);
...
static void test_gio_fallback_receives_open_and_directory(void) {
    ...
    if (ready) {
        (void)setenv("PATH", root, 1); (void)setenv("PHYSICS_SIM_PATH_OPENER_LOG", log_path, 1);
        (void)setenv("PHYSICS_SIM_PATH_OPENER_XDG", "unavailable", 1);
        expect_true("opener_gio_result", PhysicsSim_PathOpener_OpenDirectory(directory) == PHYSICS_SIM_PATH_OPENER_OPENED);
        expect_true("opener_gio_args", read_text(log_path, args, sizeof(args)) &&
                    strstr(args, "open\n") == args && strstr(args, directory) != NULL);
        cleanup_fixture(root, directory, xdg_open, gio, log_path);
    }
}

static void test_invalid_directory_does_not_run_opener(void) {
    expect_true("opener_invalid_path", PhysicsSim_PathOpener_OpenDirectory("/path/that/does/not/exist") ==
