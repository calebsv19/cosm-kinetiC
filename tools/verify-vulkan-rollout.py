...
def run_app(app: Path, initial: Path, resized: Path, log: Path,
    ...
    env["PHYSICS_SIM_VULKAN_ROLLOUT_INITIAL_CAPTURE"] = str(initial.resolve())
    env["PHYSICS_SIM_VULKAN_ROLLOUT_RESIZED_CAPTURE"] = str(resized.resolve())
    env["PHYSICS_SIM_VULKAN_ROLLOUT_MIN_SCALE"] = str(minimum_scale)
    ...
    if result.returncode != 0:
        raise SystemExit(f"PhysicsSim Vulkan self-test exited {result.returncode}")
    if "[vk_runtime validation]" in output:
        raise SystemExit("Vulkan validation emitted warning/error diagnostics")
    receipts = re.findall(
        r"PHYSICS_SIM_VULKAN_RUNTIME schema=1 stage=(startup|resized|restart) "
        r"runtime=0\.6\.0 .* shared_device=1 validation_requested=1 "
        r"validation_enabled=1 warnings=0 errors=0", output)
    if receipts != ["startup", "resized", "restart"]:
        raise SystemExit(f"incomplete runtime receipts: {receipts}")
    if not re.search(r"PHYSICS_SIM_VULKAN_ROLLOUT schema=1 status=pass\b", output):
