...
- `config/` – tracked default configuration and reference assets. Edit `app.json` to define default window/grid/timing/fluid constants and command budget baselines.
...
- `config/custom_preset.txt` – tracked default preset seed used as fallback when runtime state has not been created yet.
- `data/runtime/` – runtime-persisted mutable state (ignored): `app_state.json` and `custom_preset.txt`.
...
- `export/volume_frames/` – planar runs still emit raw `.vf2d` plus legacy `VFHD` `.pack`, while authoritative volumetric `3D` runs now auto-route to raw `.vf3d` plus `VF3H` `.pack` with density, velocity `x/y/z`, pressure, `solid_mask`, truthful `manifest.json`, and `scene_bundle.json` metadata for downstream handoff. The `vf2d` reader in `export/volume_frames.h` still understands legacy planar files for backward compatibility.
...
## Current 3D Volume Export State
- Producer-side truthful `3D` export is complete through `PSBU-11D`.
- When runtime authority is truly volumetric `XYZ`, export now writes sibling `vf3d` artifacts instead of silently downcasting to planar `vf2d`.
- The frozen first-pass `3D` contract carries:
  - raw `.vf3d`
  - additive `VF3H` `.pack`
  - arrays for density, velocity `x/y/z`, pressure, and `solid_mask`
  - truthful `manifest.json` / `scene_bundle.json` metadata with `frame_contract=vf3d` and `space_mode=3d`
- Focused proof coverage now includes:
  - `make test-volume-frames-3d-export-contract`
  - `make test-volume-frames-3d-tiny-parity-contract`
...
## Execution flow
`src/main.c` loads runtime state from `data/runtime/app_state.json` when present (otherwise falls back to `config/app.json`), then opens the SDL scene menu so you can choose simulation mode, preset slot, quality profile, and optional headless settings.
...
For the full current keybind set, including structural runtime/editor controls, see `docs/KEYBINDS.md`.
...
- Structural presets and test scenes live under `config/structural_*.txt`, and the structural runtime/editor keybinds are documented in `docs/KEYBINDS.md`.
