...
- `export/volume_frames/` – raw `.vf2d` dumps (density + velocity) written with v2 headers (grid, time, dt, origin, cell size, obstacle mask CRC) plus a `manifest.json` listing every frame. The reader in `export/volume_frames.h` still understands v1 files for backward compatibility.
