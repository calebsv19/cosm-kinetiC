...
- `config/objects/` – canonical ShapeAsset JSON files (flattened polylines only). The sim loads every `*.asset.json` here into the shared asset library at startup. Legacy `import/*.json` from the line-drawing tool can be auto-converted via the picker or `shape_asset_tool`.
