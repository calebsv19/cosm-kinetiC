...
## Subdirectories
- `app/` – configuration defaults plus the scene editor/menu, presets, scene state, and controller that glue inputs, solvers, and rendering together. The scene controller now exposes explicit IR1 input phases (`InputIntake -> InputNormalize -> InputRoute -> InputInvalidate`) and explicit RS1 frame phases (`Update -> RenderDerive -> RenderSubmit`) via typed contracts.
