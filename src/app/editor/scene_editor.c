...
bool scene_editor_run(SDL_Window *window,
    ...
    int panel_width = 280;
    int canvas_margin = 40;
    int max_canvas = winH - 220;
    if (max_canvas < 240) max_canvas = 240;
    int available_w = winW - panel_width - (canvas_margin * 2);
    if (available_w < max_canvas) {
        max_canvas = available_w;
    }
    if (max_canvas < 200) max_canvas = 200;
    state.canvas_size = max_canvas;
    state.canvas_x = canvas_margin;
    state.canvas_y = 90;

    state.panel_rect.x = state.canvas_x + state.canvas_size + 24;
    state.panel_rect.y = state.canvas_y;
    state.panel_rect.w = panel_width;
    state.panel_rect.h = state.canvas_size;
