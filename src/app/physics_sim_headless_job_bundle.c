...
bool physics_sim_headless_job_bundle_load(const char *job_json_path,
    ...
    const char *text = NULL;
    PhysicsSimHeadlessJobBundle bundle;
    ...
    if (strcmp(bundle.envelope.program, "physics_sim") != 0) {
        json_object_put(root);
        diag_set(out_diagnostics, out_diagnostics_size, "outer job program must be physics_sim");
        return false;
    }
