...
typedef struct SimRuntimeObstacleOrientedBox3D {
    int center_x;
    int center_y;
    int center_z;
    float half_x;
    ...
    int min_z;
    int max_z;
} SimRuntimeObstacleOrientedBox3D;
...
static bool build_object_box(const SimRuntime3DDomainDesc *desc,
    ...
    box.center_x = normalized_position_to_grid(object->position_x, desc->grid_w);
    box.center_y = normalized_position_to_grid(object->position_y, desc->grid_h);
    box.center_z = normalized_position_to_grid(object->position_z, desc->grid_d);
    box.half_x = clamp_positive(object->size_x * (float)desc->grid_w, 1.0f);
    box.half_y = clamp_positive(object->size_y * (float)desc->grid_h, 1.0f);
    half_z = clamp_positive(object->size_z, object->size_x);
    box.half_z = clamp_positive(half_z * (float)desc->grid_d, 1.0f);
...
static bool build_import_box(const SimRuntime3DDomainDesc *desc,
    ...
    box.center_z = normalized_position_to_grid(imp->position_z, desc->grid_d);
    box.half_x = clamp_positive(scale * SCAFFOLD_IMPORT_DESIRED_FIT * 0.5f * (float)desc->grid_w, 1.0f);
    box.half_y = clamp_positive(scale * SCAFFOLD_IMPORT_DESIRED_FIT * 0.5f * (float)desc->grid_h, 1.0f);
    box.half_z = clamp_positive(scale * SCAFFOLD_IMPORT_DESIRED_FIT * 0.5f * (float)desc->grid_d, 1.0f);
...
void backend_3d_scaffold_rasterize_retained_object_obstacles(
    ...
    for (size_t i = 0; i < scene->preset->object_count && i < MAX_PRESET_OBJECTS; ++i) {
        ...
        if (object->type == PRESET_OBJECT_CIRCLE) {
            ...
            int center_z = normalized_position_to_grid(object->position_z, state->volume.desc.grid_d);
