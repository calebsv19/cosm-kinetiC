...
package-linux-desktop-self-test: package-linux-desktop
	...
	@test -f "$(RELEASE_DIR)/linux-desktop-self-test/xdg-data/applications/kinetic.desktop" || (echo "Installer did not write kinetic.desktop"; exit 1)
	@test -f "$(RELEASE_DIR)/linux-desktop-self-test/xdg-data/icons/hicolor/scalable/apps/kinetic.svg" || (echo "Installer did not write kinetic.svg"; exit 1)
	@PHYSICS_SIM_RUNTIME_DIR="$(LINUX_DESKTOP_SELF_TEST_RUNTIME_DIR)" XDG_STATE_HOME="$(LINUX_DESKTOP_SELF_TEST_STATE_DIR)" "$(RELEASE_DIR)/linux-desktop-self-test/unpack/$(LINUX_DESKTOP_BASENAME)/bin/physics-sim-launcher" --self-test
